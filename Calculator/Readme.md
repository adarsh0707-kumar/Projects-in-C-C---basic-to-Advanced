...
- A modular architecture with 257 automated unit tests and zero exit() calls in the engine (every error path degrades gracefully instead of terminating the process)
...
├── Tests/                      # 257 unit tests (Tests/test_main.c is the test runner's entry point)
├── docs/
...
Runs all **257** unit tests covering: stack, tokenizer, validator, parser, postfix conversion, functions, variables, history, and memory. All tests also pass cleanly under AddressSanitizer and UndefinedBehaviorSanitizer (`make asan && make BUILD=asan test`).
